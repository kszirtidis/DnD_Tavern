/* Citation for the following update_transaction.js function:
    Date: 2/29/2024 
    Adapted from code provided by Prof. Safonte and Dr. Curry for students at Oregon State University. Function
    was adapted and changed according to the needs of our database. Some comments were modified or removed.
    Source: https://github.com/osu-cs340-ecampus/nodejs-starter-app
*/



// Get the objects we need to modify
let updateTransactionForm = document.getElementById('update-transaction-form-ajax');

// Modify the objects we need
updateTransactionForm.addEventListener("submit", function (e) {
   
    // Prevent the form from submitting
    e.preventDefault();

    // Get form fields we need to get data from
    let inputtransaction = document.getElementById("mySelect");
    let inputguest = document.getElementById("input-guestID-update");
    let inputitem = document.getElementById("input-itemID-update");
    let inputquantity = document.getElementById("input-quantity-update");
   
    let inputtransactiondate = document.getElementById("input-transactionDate-update");


    // Get the values from the form fields
    let transaction = inputtransaction.value;
    let guest = inputguest.value;
    let item = inputitem.value;
    let quantity = inputquantity.value;
   
    let date = inputtransactiondate.value;

    // currently the database table for bsg_people does not allow updating values to NULL
    // so we must abort if being bassed NULL for transaction

    if (isNaN(transaction, guest, item, quantity, date)) 
    {
        return;
    }


    // Put our data we want to send in a javascript object
    let data = {
        transaction: inputtransaction.value,
        guest: inputguest.value,
        item: inputitem.value,
        quantity: inputquantity.value,
     
        date: inputtransactiondate.value
       
    }
    
    // Setup our AJAX request
    var xhttp = new XMLHttpRequest();
    xhttp.open("PUT", "/put-transaction-ajax", true);
    xhttp.setRequestHeader("Content-type", "application/json");

    // Tell our AJAX request how to resolve
    xhttp.onreadystatechange = () => {
        if (xhttp.readyState == 4 && xhttp.status == 200) {

            // Add the new data to the table
            updateRow(xhttp.response, transaction, guest, item, quantity, date);

            location.reload();

        }
        else if (xhttp.readyState == 4 && xhttp.status != 200) {
            console.log("There was an error with the input.")
        }
    }

    // Send the request and wait for the response
    xhttp.send(JSON.stringify(data));

})


function updateRow(data, transactionID){
    let parsedData = JSON.parse(data);
    
    let table = document.getElementById("transactions-table");

    for (let i = 0, row; row = table.rows[i]; i++) {
       //iterate through rows
       //rows would be accessed using the "row" variable assigned in the for loop
       if (table.rows[i].getAttribute("data-value") == transactionID) {

            // Get the location of the row where we found the matching transaction ID
            let updateRowIndex = table.getElementsByTagName("tr")[i];

            // Get td of room value
            let td = updateRowIndex.getElementsByTagName("td")[3];

            // Reassign room to our value we updated to
            td.innerHTML = parsedData[0].name; 
       }
    }
}
